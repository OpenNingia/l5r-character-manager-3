...
Dialog {
    ...
    function _syncFamilyCombo() {
        ...
        for (var i = 0; i < _families.length; ++i) {
            if (_families[i].id === _selectedFamilyId) { idx = i; break }
        }
        familyCombo.currentIndex = idx >= 0 ? idx : 0
        _selectedFamilyId = familyCombo.count > 0
            ? _families[familyCombo.currentIndex].id : ""
    }

    function _currentFamilyRecord() {
        if (familyCombo.currentIndex < 0
            || familyCombo.currentIndex >= _families.length) return null
