...
Pane {
    ...
    readonly property var _rings:  pcProxy ? pcProxy.rings : ({ earth: 0 })

    // Parchment surface + ink-on-paper palette, identical to SheetPanel.
    background: Rectangle { color: Theme.parchment; radius: 0 }
    ...
    function _nameFor(idx) {
        const names = [qsTr("Healthy"), qsTr("Nicked"), qsTr("Grazed"),
                       qsTr("Hurt"), qsTr("Injured"), qsTr("Crippled"),
                       qsTr("Down"), qsTr("Out")]
    ...
    function _cardBg(idx, active) {
        if (idx === 7) return active ? "#4a1e16" : "#5a261c"
        if (idx === 6) return active ? "#d99090" : "#f0c8c8"
        if (active) return Theme.positive
        return Theme.parchmentInset
    }
    function _cardInk(idx, active) {
        if (idx === 7) return "#f6e6c8"
        if (idx === 6) return active ? "#3a0e0e" : "#5a1a1a"
        if (active) return "#fdf6e3"
        return Theme.ink
    }
    function _bucketStart(idx) {
        if (idx <= 0 || !panel._wounds[idx - 1]) return 0
        return panel._wounds[idx - 1].value
    }
    function _statusCaption() {
        if (panel._lvl >= 7)   return qsTr("unconscious")
        if (panel._cur === 0)  return qsTr("no wounds taken")
    ...
    contentItem: ColumnLayout {
        ...
        RowLayout {
            ...
            Label {
                ...
                ToolTip.delay: 300
                ToolTip.text: qsTr(
                    "Cell layout: name · threshold · TN penalty · wounds in level\n" +
            ...
            RankStepper {
                ...
                onValueModified: function(v) {
                    if (appCtrl) appCtrl.setHealthMultiplier(v)
                }
            }
            ...
            RankStepper {
                ...
                onValueModified: function(v) {
                    if (appCtrl) appCtrl.setWoundsTotal(v)
                }
            }
        }
        ...
        Rectangle {
            ...
            RowLayout {
                ...
                Label {
                    ...
                    color: panel._lvl >= 6 ? Theme.accentMuted
                                           : (panel._lvl === 0 ? Theme.positive
                                                                : Theme.heading)
                }
                Label {
                    text: (panel._wounds[panel._lvl] && panel._wounds[panel._lvl].penalty > 0)
                        ? qsTr("· +%1 TN to all rolls")
                            .arg(panel._wounds[panel._lvl].penalty)
                        : qsTr("· no penalty")
                    font.pixelSize: Theme.smallFont
                    opacity: 0.85
                }
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            Label {
                                text: index === 7
                                    ? qsTr("unconscious")
                                    : qsTr("+%1 TN").arg(modelData.penalty)
                                font.pixelSize: Theme.smallFont
