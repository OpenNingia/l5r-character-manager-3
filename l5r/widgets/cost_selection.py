...
class XpTextBox(QtGui.QLineEdit):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(event.rect().width() - right_margin -
                         tx_rect.width(), event.rect().top() + tx_rect.height(), self.sub_text)
...
class CostSelection(QtGui.QWidget):
    ...
    def update_suggested_text(self):
        if self.reason:
            self.tx_suggested.setText(
                '{0} [{1}]'.format(self.suggested, self.reason))
        else:
