...
class SchoolChoiceDlg(QtGui.QDialog):
    ...
    def build_ui(self):
        ...
        def build_filter_panel():
            ...
            self.cx_base_schools = QtGui.QCheckBox(
                self.tr("Base schools"), self)
            self.cx_advc_schools = QtGui.QCheckBox(
                self.tr("Advanced schools"), self)
            self.cx_path_schools = QtGui.QCheckBox(
                self.tr("Alternate paths"), self)
        ...
        self.bt_box.addButton(
            self.tr("Cancel"), QtGui.QDialogButtonBox.RejectRole)
        self.bt_box.addButton(
            self.tr("Confirm"), QtGui.QDialogButtonBox.AcceptRole)
    ...
    def refresh_data(self):
        ...
        if self.cx_path_schools.isChecked():
            schools += [
                x for x in self.dstore.schools if 'alternate' in x.tags]
    ...
    def on_school_change(self):
        ...
        if clan and school:
            ...
            self.req_list.set_requirements(
                self.pc, self.dstore, school.require)
    ...
    def on_accept(self):
        ...
        if self.school_id:
            if not self.req_list.match():
                ...
                msgBox.setText(
                    self.tr("You don't have the requirements to join this school."))
                msgBox.exec_()
            else:
                self.accept()
