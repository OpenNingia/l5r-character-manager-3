...
class AdvancementViewModel(QtCore.QAbstractListModel):
    ...
    def advancement_human_string(self, adv):
        if adv.type == 'memo_spell':
            spell = api.data.spells.get(adv.spell_id)
            if not spell:
                return 'not_found'
            return self.tr('{0}, Mastery {1}. Cost: {2} xp'.format(
                spell.name, spell.mastery, adv.cost))
        if adv.type == 'kiho':
            kiho = api.data.powers.get_kiho(adv.kiho)
            if not kiho:
                return 'not_found'
            return self.tr('{0}. Cost: {1} xp'.format(
                kiho.name, adv.cost))
        return 'TODO'
    ...
    def add_item(self, item):
        ...
        item.desc = self.advancement_human_string()
...
class AdvancementItemDelegate(QtGui.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        # paint the type with a smaller font over the item name
